...
all:
  hosts:
    ${yandex_compute_instance.node["sonar-01"].name}:
      ansible_host: ${yandex_compute_instance.node["sonar-01"].network_interface.0.nat_ip_address}
    ${yandex_compute_instance.node["nexus-01"].name}:
      ansible_host: ${yandex_compute_instance.node["nexus-01"].network_interface.0.nat_ip_address}
  children:
    sonarqube:
      hosts:
        ${yandex_compute_instance.node["sonar-01"].name}:
