...
resource "local_file" "inventory" {
  content = <<-DOC
    ...
    [clickhouse]
    ${yandex_compute_instance.node["clickhouse"].hostname} ansible_host=${yandex_compute_instance.node["clickhouse"].network_interface.0.nat_ip_address}
    [clickhouse:vars]
    ansible_user=${var.ssh_user}
    ansible_ssh_private_key_file="~/.ssh/id_yc_nodes"

    [vector]
    ${yandex_compute_instance.node["vector"].hostname} ansible_host=${yandex_compute_instance.node["vector"].network_interface.0.nat_ip_address}
    [vector:vars]
    ansible_user=${var.ssh_user}
    ansible_ssh_private_key_file="~/.ssh/id_yc_nodes"

    [lighthouse]
    ${yandex_compute_instance.node["lighthouse"].hostname} ansible_host=${yandex_compute_instance.node["lighthouse"].network_interface.0.nat_ip_address}
    [lighthouse:vars]
    ansible_user=${var.ssh_user}
    ansible_ssh_private_key_file="~/.ssh/id_yc_nodes"
